...
final class GoogleTests: XCTestCase {
	...
	func testSuppliedParameters() async throws {
		let googleParameters = GoogleAPI.GoogleAPIParameters(
			includeGrantedScopes: true, loginHint: "john@doe.com")
		...
		let creds = AppCredentials(
			clientId: "client_id", clientPassword: "client_pwd", scopes: ["scope1", "scope2"],
		...
		XCTAssertTrue(
			urlComponent!.queryItems!.contains(where: { $0.name == GoogleAPI.includeGrantedScopeKey }))
